...
public class BackIntakeMotor extends SubsystemBase{
    ...
    public BackIntakeMotor() {
        ...
        BackIntakeMotor = new TalonFX(15, "DriveCan");
        BackIntakeMotor.getConfigurator().apply(TalonFXConfig);
        ...
        for (int i = 0; i < 5; ++i) {
            status = BackIntakeMotor.getConfigurator().apply(TalonFXConfig);
    ...
    public void periodic() {
                if (m_printCount++ > 10) {
            m_printCount = 0;
            SmartDashboard.putNumber("Velocity", BackIntakeMotor.getVelocity().getValueAsDouble());
          }

    }
    public void setIntakeSpeed(double velSetpoint, double accSetpoint){
        BackIntakeMotor.setControl(m_vdReq.withVelocity(velSetpoint).withSlot(0).withAcceleration(accSetpoint).withSlot(0).withEnableFOC(true));
        // Ryan - give this FOC profile a try
        //        BackIntakeMotor.setControl(m_vdFOCReq.withVelocity(velSetpoint).withSlot(0).withAcceleration(accSetpoint).withSlot(0));
      }

    public void BackIntakeMotorOneRun(double setpoint){
        BackIntakeMotor.set(setpoint);
        // BackIntakeMotor2.set(setpoint);
    }
    ...
    private final SysIdRoutine m_sysIdRoutine = new SysIdRoutine(
        ...
        new SysIdRoutine.Mechanism(
            ...
            log -> {
                log.motor("Intake")
                    .voltage(Volts.of(BackIntakeMotor.getMotorVoltage().getValueAsDouble()))
                    .angularPosition(Rotations.of(BackIntakeMotor.getPosition().getValueAsDouble()))
                    .angularVelocity(RotationsPerSecond.of(BackIntakeMotor.getVelocity().getValueAsDouble()));
            },
            this
        )
    );
